...
def search(
    ...
    try:
        ...
        for blob in src_container.list_blobs():
            ...
            for _, row in df.iterrows():
                ...
                # NEW: Field-based exact matching logic
                ...
                # If no structured fields provided, fall back to extracted from address string
                ...
                logging.debug("Matching | user: num=%s name=%s zip=%s | rec: num=%s name=%s zip=%s",
                             user_street_number, user_street_name, user_zip,
                             rec_street_number, rec_street_name, rec_zip)
                
                # Match logic: only match fields that user provided
                matched = True
                ...
                if matched and user_street_name:
                    if not rec_street_name:
                        ...
                        # Normalize both and check if they match
                        # For exact matching: all user tokens should be present in record
                        user_tokens = [t for t in user_street_name.split() if t]
                        rec_tokens = [t for t in rec_street_name.split() if t]
                        
                        if len(user_tokens) > 0:
                            # Check if all user tokens are present in record tokens
                            # Allow for slight variations (e.g., "st" vs "street")
                            all_found = True
                ...
                if matched and user_zip:
                    if not rec_zip:
                        ...
                        # Zip codes should match exactly (first 5 digits)
                        user_zip_clean = user_zip.strip()[:5]
                        rec_zip_clean = rec_zip.strip()[:5]
                        if user_zip_clean != rec_zip_clean:
                            matched = False
                
                # If no fields were provided, fall back to old matching logic
                if not user_street_number and not user_street_name and not user_zip:
                    ...
                    if not rec_addrs:
                        ...
                        for cand in rec_addrs:
                            ...
                            if score >= 3:
                                matched = True
                                if not matched_by or matched_by == "field_match":
                                    matched_by = "score"
                                break
    ...
    if len(results_sorted) == 0:
        logging.info("SEARCH done | No records found matching the provided address fields")
        return JSONResponse({
            "results": [],
            "message": "Record not found",
            "error": "No records match the provided address fields. Please verify the street number, street name, and postal code.",
            "duration_ms": dur_ms,
            "canonical_hit": False
        })
